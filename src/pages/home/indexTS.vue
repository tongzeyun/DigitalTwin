	 ...
	 <!-- 
      ...
      <div class="action-buttons">
				坐标：{{worldSignalPos}}
				  ...
				  <el-form :inline="true" :model="formInline" class="demo-form-inline">
				    ...
				    <el-form-item label="y">
				      <el-input v-model="formInline.y" placeholder="y" clearable />
				    </el-form-item>
					<el-form-item label="z">
					  <el-input v-model="formInline.z" placeholder="z" clearable />
					</el-form-item>
					<br/>
					<el-form-item label="ax">
					  <el-input v-model="formInline.ax" placeholder="ax" clearable />
					</el-form-item>
					<el-form-item label="ay">
					  <el-input v-model="formInline.ay" placeholder="ay" clearable />
					</el-form-item>
					<el-form-item label="az">
					  <el-input v-model="formInline.az" placeholder="az" clearable />
					</el-form-item>
				    <el-form-item>
				...
				<el-tooltip
				        class="box-item"
				        effect="dark"
				        content="x左右偏移视角（ax>x 右侧角度，反之左侧）  ax与az配合可实现侧面视角----------
								y高度视角（ay>y 俯视角度）--------
								z前后距离视角（az>z 正面角度，反之背面）--------- 
...
const onSubmit = () => {
  ...
  cameraTween(formInline.x,formInline.y,formInline.z,formInline.ax,formInline.ay,formInline.az)
}
...
function loadAnimation(type) {
  if (gltfs.animations && gltfs.animations.length > 0) {
	...
	newArr = ['trl','zhlou','djl','bnw3','wg3','all']
	...
	// 			playAnimation(anis2,num)
	// 	}
	// 	if (newStr === type) { //相等，关闭点击的模型
	
	// 				num=-1
	// 				let anis2 = gltfs.animations.slice(0,);
	// 				playAnimation(anis2,num)
	// 	}
	// 	if(!newStr || newStr !== type){//不存在 或者 不相等，打开选中的模型
	// 		newStr=type
	// 		num=1
	// 		playAnimation(anis,num)
	// 	}
...
function addClickEventListener() {
  ...
  function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
      ...
      //检测鼠标点击交点
       worldSignalPos = intersects[0].point.x; // 射线与地面的交点（世界坐标）
      console.log('鼠标点击交点坐标:',object.name, worldSignalPos )
		
		
      // 跳转路由  第五层 
      if(object.name === '立方体076'||object.name === '立方体077'||object.name === '立方体077_5'){ //第五层 
		
		if(num==1 && newStr==="trl"){ //陶然楼打开才能运行
			cameraTween(100,3,35,100,40,35)
...
function visibleFalse(obj,i,name){
	// 相机飞行到的位置和观察目标拉开一定的距离

	if(i==1){
		model.traverse((child) => {
		    if (child.isMesh && child.material) {
				 if(child.name.indexOf('立方体076')!==-1){ //obj.name){ //模糊匹配去掉第5层的屋顶
					let caiZhi = child.clone()//克隆，防止影响的原模型材质
						caiZhi.material.visible=false
						child.material.copy(caiZhi.material)
