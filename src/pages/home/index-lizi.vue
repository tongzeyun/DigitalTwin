...
function initThreeScene() {
  ...
  // 添加UnrealBloomPass用于辉光效果
   const bloomPass = new UnrealBloomPass(
     new THREE.Vector2(width, height),
     0.8,  // 增加强度，增强整体辉光效果
     0.8,  // 增加半径，使辉光范围更广
     0.9  // 调整阈值到合理范围，确保每个粒子都能产生均匀的辉光效果
   );
      ...
      (gltf) => {
        ...
        model.traverse((child) => {
          if (child.isMesh && child.material) {
            ...
            if (child.name === '玻璃001' || child.name === '立方体011' || child.name === '立方体007' || child.name === '立方体008' || child.name === '立方体009' || child.name === '玻璃002' || child.name === '玻璃004' && !Array.isArray(child.material)) {
            ...
            // Rectangle121003  玻璃  设置默认材质属性****
            // if (child.name === 'Rectangle121003' && !Array.isArray(child.material)) {
            //   child.material.color.set(0x55739f)
            //   child.material.opacity = 0.6
            //   child.material.roughness = 0.5
            //   child.material.metalness = 0.5
            ...
            //  立方体074_1 墙体  设置默认材质属性****
            // if (child.name === '立方体074_1' || child.name === '立方体137_1' || child.name === '立方体142' || child.name === '墙面' || child.name === '立方体057_1' && !Array.isArray(child.material)) {
            //   child.material.color.set(0xffddb2)
            ...
            // }
            // 18房顶 材质 
            // if (child.name === '18房顶' || child.name === '18房顶001' || child.name === '立方体120_2' || child.name === '立方体081_2' || child.name === '立方体079_2' || child.name === 'Rectangle220001' || child.name === '柱体004_2' || child.name === '网格2457_1' || child.name === '立方体082_2' && !Array.isArray(child.material)) {
            //   child.material.color.set(0x8d0c0c)
            //   child.material.opacity = 1
            //   child.material.roughness = 0.5
            //   child.material.metalness = 0.5
            //   // 确保材质支持透明度
            ...
            // }
            //金属球  立方体186_2
            // if (child.name === '立方体135_2' && !Array.isArray(child.material)) {
            //   child.material.color.set(0xffad33)
            //   child.material.opacity = 1
  ...
  function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
      ...
      if (object.isMesh && object.material) {
        ...
        //   // 当点击立方体076_1时，让一组特定模型一起动画
        //   const targetModelNames = [
        //     '立方体076_1', '立方体076', '立方体003_1',
        //     '立方体076_2', '立方体003_2', '立方体003_9',
        //     '立方体003_10', '立方体003_8', '立方体003_5',
        //     '立方体003_4', '立方体003_6'
        //   ]

        //   // 查找并添加所有目标模型
        //   scene.traverse((child) => {
        //     if (child.isMesh && targetModelNames.includes(child.name) && !animatingObjects.value.includes(child)) {
        //       // 存储原始位置
        ...
        //       // 添加到动画列表
        //       animatingObjects.value.push(child)
        //       console.log(`添加模型到动画列表: ${child.name}`)
        //     }
        //   })
        // }
        //三层
        if (object.name === '立方体009') {
          // 当点击立方体009时，让一组特定模型一起动画
          const targetModelNames = [
            '立方体009_1', '立方体009', '立方体003_3',
            '立方体009_2', '立方体009_3', '立方体009_4',
            '立方体009_5', '立方体009_9', '立方体009_7',
            '立方体009_8'
          ]
        ...
        //二层
        if(object.name === '立方体010_1'){

          // 当点击立方体010_1时，让一组特定模型一起动画
          const targetModelNames = [
            '立方体010_1', '立方体010', 
            '立方体010_2', '立方体010_3', '立方体010_4', 
            '立方体010_5', '立方体010_9', '立方体010_10', 
            '立方体010_8'
          ]
...
function onWindowResize() {
  if (!camera || !renderer || !modelContainer.value || !labelRenderer || !composer) return
...
function animate() {
  ...
  // 粒子围栏呼吸动画效果 - 优化以确保辉光均匀显示
  if (particleMaterial) {
    const time = Date.now() * 1 // 使用更平滑的时间因子
    // 计算呼吸效果的脉冲因子 - 更缓慢的呼吸节奏
    const pulseFactor = 0.5 + 0.5 * Math.sin(time)
    
    // 调整透明度，保持平滑的呼吸效果
    particleMaterial.opacity = 0.7 + pulseFactor * 0.2 // 透明度在0.7-0.9之间变化
...
function useSpecifiedEdgePoints() {
  ...
  const specifiedPoints = [
   ...
   {x: 138.72515410586828, y: 11, z: 36.791260087803124},
{x: 107.2514106757945, y: 11, z: 30.41059361282671},
{x: 80.31058272818166, y: 11, z: 31.17130870903089},
{x: 81.6689325689534, y: 11, z: 40.561014604875766},
{x: 83.3154059281424, y: 11, z: 41.18710742286021},
{x: 82.3180637225627, y: 11, z: 50.919903968183775},
{x: 91.19263114513063, y:11, z: 51.49937438964844},
{x: 91.34807472554479, y: 11, z: 58.92853518447659},
{x: 101.53728856265616, y: 11, z: 59.51585047684284},
{x: 127.18782043457031, y: 11, z: 53.051344650027445},
{x: 138.72515410586828, y: 11, z: 36.791260087803124},
   // 闭合围栏
  ]
  ...
  // 线性插值增加点密度
  const segmentPoints = 180 // 每段插入的点数
  
  for (let i = 0; i < specifiedPoints.length - 1; i++) {
    const start = specifiedPoints[i]
    const end = specifiedPoints[i + 1]
  ...
  const edgePoints = tempPoints
  console.log('使用指定边缘点创建围栏，总点数:', edgePoints.length)
  return edgePoints
}
...
function createParticleFence() {
  ...
  // 获取边缘点
  const edgePoints = useSpecifiedEdgePoints()
  
  // 检查边缘点是否有效
  if (!edgePoints || edgePoints.length === 0) {
    console.warn('没有有效的边缘点用于创建粒子围栏')
    return
  }
  
  // 创建粒子几何体
  const geometry = new THREE.BufferGeometry()
  const particleCount = edgePoints.length * 3 // 每个边缘点生成3个粒子
  ...
  for (let i = 0; i < particleCount; i++) {
    const pointIndex = Math.floor(i / 3) % edgePoints.length
    const point = edgePoints[pointIndex]
    
    // 设置粒子位置（带有微小随机偏移）
    positions[i * 3] = point.x + (Math.random() - 0.5) * 0.5
    positions[i * 3 + 1] = point.y // 使用原始y值，不添加额外偏移
    positions[i * 3 + 2] = point.z + (Math.random() - 0.5) * 0.5
    
    // 设置粒子大小（使用固定大小以确保每个粒子显示一致的辉光效果）
    sizes[i] = 9.0
  ...
  particleMaterial = new THREE.PointsMaterial({
      color: 0xff0000, // 纯红色作为基础颜色
      size: 2, // 使用非常小的固定大小，因为我们会通过size属性数组设置实际大小
      sizeAttenuation: false, // 禁用大小衰减，使粒子在不同距离保持相同大小
      transparent: true,
      opacity: 0.9, // 保持高透明度
      blending: THREE.AdditiveBlending, // 叠加混合模式增强辉光效果
      emissive: 0xff0000, // 使用与基础色相同的颜色以确保辉光一致性
      emissiveIntensity: 15.0, // 调整发光强度以增强均匀辉光效果
      depthWrite: false, // 避免深度写入问题
      alphaTest: 0.01, // 降低alphaTest阈值，改善边缘效果
      fog: false // 禁用雾效，保持粒子清晰度
    })
  ...
  if (scene) {
    ...
    console.log('粒子围栏创建成功，粒子数量:', particleCount)
  }
}
