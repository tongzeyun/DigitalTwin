  ...
  <div class="digital-twin-container">
    <div class="header-section">
	 ...
	 <el-button @click="toDigital">原点</el-button>
	  <el-button @click="toDigital2">陶然楼</el-button>
...
// '/frtwin/area-test.glb',/frtwin/trl-model/b1b2/b1b2.glb
    ...
    //loader.load('/frtwin/trl-model/b1b2/b1b2.glb',
      (gltf) => {
		...
		// 	console.log('模型材质太多,不好找对应材质颜色,可以动画这里改，目前动画桃源居每层不在一起，不好给单独颜色')
		// console.log('材质:', model)
		// 	console.log('模型包含动画:', gltf)
		// 	// console.log(gltf,model,model.children[0].children[5])////model.children[0].children[1].name: "立方体077_3"
		// 	// model.children[0].children[5].material.color={isColor: true, r: 1, g: 0.9, b: 0.4}
		// }
        // 初始化动画混合器并存储动画数据
        // loadAnimation();
...
function loadAnimation(type) {
  if (gltfs.animations && gltfs.animations.length > 0) {
	...
	switch(type){
			case "sanCheng"://console.log('播放动画，所有');
				anis = gltfs.animations.slice(0,4);
			break;
			case "trl"://console.log('播放陶然楼模型的动画');
				anis = gltfs.animations.slice(4, 11);
			break;
			case "zhlou"://console.log('播放综合楼1的动画');
				anis = gltfs.animations.slice(7, 34);
				break;
			case "djl":
				anis = gltfs.animations.slice(35,54);
				break;
			case "bnw3":
				anis = gltfs.animations.slice(55,64);
				break;
			case "wg3"://console.log('网格的动画，所有');
				anis = gltfs.animations.slice(55,64);
				break;
			case "all"://console.log('播放门岗模型的动画，所有');
				anis = gltfs.animations.slice(4,);
				break;
	...
	if (newArr.indexOf(type) === -1) {
			...
			playStatic=false
			num=-1
	} //附加与去重复，目的是为了只能点击一次，再点就关闭动画
	// console.log('sssss',newArr,playStatic); // 结果：
	
	mixer = new THREE.AnimationMixer(model)
	
	
	
    // 遍历所有动画并为每个包含动画的对象存储动画动作
    anis.forEach((animation, index) => {
		let aname = animation.name
		 // console.log(`动画 ${index}:`, aname)
		 
		

	  if(aname.indexOf('摄像机') !== -1){//if(aname.indexOf('墙体') !== -1){// 陶然楼五楼摄像机动作
		  console.log(`动画 ${index}:11111111111111111111111`, animation)

					
		  		// 2. 提取模型内置摄像机（根据节点名称匹配）
		  		 // let camera2 = THREE.PerspectiveCamera //| THREE.OrthographicCamera; // 模型内置摄像机
		  		// f5_taoran_donghua004  摄像机004
				
				// let pc = gltfs.cameras[0]
				console.log('camera',camera)
		  		
				// camera = gltfs.cameras.getObjectByName('f3_taoran_donghua001') as THREE.PerspectiveCamera();//踩坑：相机位置，方法有问题

					gltfs.cameras.map((item,i)=>{													//方法2.完善赋值相机，完全赋值，但效果依旧与预期不同
						
						// if(i==6){
						// 	// child.material.visible=false
						// 	item=""
						// }
						if(item.name === "f2_taoran_donghua002"){//f3_taoran_donghua001//f1_taoran_donghua003//f2_taoran_donghua002
							camera = item
							// const pos =new THREE.Vector3(item.position);//140.5.60// new THREE.Vector3(7,59,138);
							// // console.log(pos)
							// // // 相机飞行到的位置和观察目标拉开一定的距离,0正面，30则偏移30度
							// // const pos2 =new THREE.Vector3(item.position);//140.5.60// new THREE.Vector3(7,59,138);
					...
					// // 若获取的是Object3D，手动包装为透视相机
					// const cameraObj = gltfs.cameras[0].getObjectByName('f3_taoran_donghua001');  //方法3.// 复制相机位置、旋转等属性
					...
					//   1000
					// );
					// camera.copy(cameraObj); // 复制位置、旋转等属性
					
					// console.log('1',gltfs.animations[0].tracks); 
		  		if (!camera) {
		  		  console.error('未找到内置摄像机节点，请检查节点名称');
		  		  return;
		  		}
		  // 		console.log('d',camera)	
				//  // 4. 查找并播放摄像机动作（根据动画名称匹配）
				    // const cameraAction: AnimationAction | undefined = gltfs.animations
				    //   .map(clip => mixer.clipAction(clip))
				    //   .find(action => action._clip.name === '陶然楼三楼摄像机动作'); // 替换为你的动画名称
					
				// const action1 = mixer.clipAction(gltfs.animations.find(c => c.name === '陶然楼三楼摄像机动作'));
				...
				//   action1.clampWhenFinished = true;//playStatic;
				// action1.play(); 
...
function playAnimation(animation,num){
	console.log('ssss',animation.name)
	let activeAction = mixer.clipAction(animation);
	...
	// console.log('sssss1',newArr,playStatic); // 结果：
	  ...
	  // 存储动画动作
	  const actions = mixer.clipAction(animation)
	  actions.play();
}
function toDigital(){
...
function toDigital2(){
	// render();
		...
		// 			   scene.translateZ(0.4)
		// 			}, 10);
		// }
		// console.log(camera.position)//7,59,138
		
		
		
	 const pos =new THREE.Vector3(95,6,70);//140.5.60// new THREE.Vector3(7,59,138);
	 // console.log(pos)
	 // // 相机飞行到的位置和观察目标拉开一定的距离,0正面，30则偏移30度
	 const pos2 =new THREE.Vector3(95,8,80);//140.5.60// new THREE.Vector3(7,59,138);
	 // const pos2 = pos.clone().addScalar(0)//.addScalar(0);//multiplyScalar(2)
	 // // 相机从当前位置camera.position飞行三维场景中某个世界坐标附近
	 // console.log(pos2,pos)
	 createCameraTween(pos2, pos)
...
function addClickEventListener() {
  ...
  function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
      ...
      console.log('鼠标点击交点坐标:',object.name, worldSignalPos)
		
		
      // 跳转路由  第五层 
      if(object.name === '立方体076'||object.name === '立方体077'){//第五层 
		loadAnimation('sanCheng')//visibleFalse(object,1)
        // router.push( '/trfifth' )
      }else if(object.name === '立方体081'){// 第三层
		 
		// let caiZhi = object.material.clone()//克隆
		// caiZhi.opacity=0.1
		// caiZhi.transparent=true
		// object.material = caiZhi
	
		// visibleFalse(object,1)
		loadAnimation('sanCheng')
		
      }else if(object.name === '立方体071'||object.name === '立方体007'){ // 第二层
		loadAnimation('sanCheng')//visibleFalse(object,1)
			// router.push( '/trsecond' )
      }else if(object.name === '柱体007_1'){ // 第一层
		loadAnimation('sanCheng')//visibleFalse(object,1)
			// router.push( '/trfirst' )
      }else{
		 visibleFalse(object,2) 
	  }

      // 负一层
      if(object.name === '地面001'){
        // router.push( '/trfourth' )
		visibleFalse(object,2) 
      }
	  ...
	  switch(object.name){
	  		...
	  		case "立方体076_1"://console.log('播放陶然楼模型的动画');
	  			aniType = 'trl';
	  		break;
	  		case "立方体017_1"://console.log('播放综合楼的动画');
	  			aniType = 'zhlou';
	  			break;
	  		case "立方体017_2"://console.log('播放综合楼的动画');
	  			aniType = 'zhlou';
	  			break;
	  		case "聚德楼二楼屋顶02":
	  			aniType = 'djl';
	  		...
	  		case "网格148_3":
	  			aniType = 'wg3';
	  			break;
	  		case "柱体017_1"://console.log('播放门岗模型的动画，所有');
	  			aniType = 'all';
	  			break;
	  		// default:
	  	 }
...
function visibleFalse(obj,i){
	// 相机飞行到的位置和观察目标拉开一定的距离
	// const pos = worldSignalPos;//new THREE.Vector3();
		...
		model.traverse((child) => {
		    if (child.isMesh && child.material) {
				 ...
				 if(child.name != obj.name){
					let caiZhi = child.material.clone()//克隆
							caiZhi.opacity=0.1
							caiZhi.transparent=true
							child.material = caiZhi
				 }
			}
		})
