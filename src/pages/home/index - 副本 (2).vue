    ...
    labelElement.onclick = (event) => {
      ...
      if (label.route) {
        ...
        router.push(label.route)
		  ...
		  // const pos =new THREE.Vector3(label.position);//140.5.60// new THREE.Vector3(7,59,138);
		  // console.log(pos)
		  // // 相机飞行到的位置和观察目标拉开一定的距离,0正面，30则偏移30度
		  const position = label.position
		  const pos =new THREE.Vector3(position.x,position.y,position.z);//140.5.60// new THREE.Vector3(7,59,138);
		  // const pos2 = pos.clone().addScalar(0)//.addScalar(0);//multiplyScalar(2)
		  // // 相机从当前位置camera.position飞行三维场景中某个世界坐标附近
		  // console.log(pos2,pos)
