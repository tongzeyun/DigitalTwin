...
function loadAnimation() {
  if (gltfs.animations && gltfs.animations.length > 0) {
    ...
    console.log('模型包含动画:', gltfs.animations)

    // 遍历所有动画并为每个包含动画的对象存储动画动作
    gltfs.animations.forEach((animation, index) => {
    ...
    let activeAction = mixer.clipAction(animation);
      ...
      // 为动画名称和整个模型都存储动画
      animations.set(animation.name, actions)
      animations.set('all', actions)
      console.log(`存储动画: ${animation.name}`)
      console.log(`当前动画存储:`, Array.from(animations.keys()))
    })
  }
}
